...
Zwischen der Modellierung und der letztendlichen Ausführung eines DSL-Modells müssen einige Verarbeitungsschritte stattfinden. Abbildung [IMAGE] veranschaulicht den Vorgang, der ein Konversationsrouting von der Spezifikation bis zur Ausführung durchläuft. Am Anfang steht die Modellierung durch den Benutzer. Hier werden die Instruktionen in einem oben beschriebenen Graphen angeordnet und Verbindungen gezogen um einen Konversationsroutingablauf zu spezifizieren. Im Editor wird synchron zu der graphischen Repräsentation eine C\#-Datenstruktur erstellt, welche die abstrakte Syntax als .NET Objekt abbildet (Details zu diesem Vorgang folgen in Kapitel \ref{sec:Editor}). 
